...
def fill_wire_cords(input_directions, fill_list, cord_name=None):
    cursor = db.cursor()
    sql = """INSERT INTO cords (wire, x_cord, y_cord) VALUES (?,?,?)
    """
    current_x = 0
    current_y = 0
    for value in input_directions.split(','):
        ...
        if re.match('^R{1}', value):
            ...
            for i in range(current_x+1, current_x+distance+1, 1):
                # print('insert {},{}'.format(i, current_y))
                cursor.execute(sql, (cord_name, i, current_y))
                fill_list.append({
                    'x': i,
                    'y': current_y
                })
        ...
        if re.match('^U{1}', value):
            ...
            for i in range(current_y+1, current_y+distance+1, 1):
                # print('insert {},{}'.format(current_x, i))
                cursor.execute(sql, (cord_name, current_x, i))
                fill_list.append({
                    'x': current_x,
                    'y': i
                })
...
fill_wire_cords(input_1, wire_one_cords, 'wire_one')
fill_wire_cords(input_2, wire_two_cords, 'wire_two')

print('len(wire_one_cords): {}'.format(len(wire_one_cords)))
print('len(wire_two_cords): {}'.format(len(wire_two_cords)))
